...
try {
    ...
    $authSettings = @{
        properties = @{
            ...
            globalValidation = @{
                ...
                excludedPaths = @('/api/v1/signing-key')
            }
            httpSettings = @{
                requireHttps = $true
            }
    ...
    if ($PublishFunctionCode) {
        ...
        try {
            & /usr/bin/zip -r $zipPath . | Out-Null
        }
        finally {
            Pop-Location
        }
    ...
    $keyResponse = Invoke-RestMethod -Uri "https://$FunctionAppName.azurewebsites.net/api/v1/signing-key" -Method Get -TimeoutSec 15
    if (-not $keyResponse.n -or -not $keyResponse.e) {
        throw 'The Function signing key endpoint returned no RSA public key material.'
    }
    $managedConfig = [ordered]@{
        ...
        SigningKeyModulus = $keyResponse.n
        SigningKeyExponent = $keyResponse.e
    ...
    $helperConfigPath = Join-Path $PSScriptRoot "QuickElevate-Intune-postinstall.generated.sh"
    @"
#!/bin/zsh
set -euo pipefail

# Generated by setup-quickelevate.ps1. Contains only public grant-verification material.
CONFIG_DIR="/Library/Application Support/QuickElevate"
CONFIG_PATH="`$CONFIG_DIR/grant-verifier.json"
...
config = {
    "grantIssuer": "$($managedConfig.GrantIssuer)",
    "grantAudience": "$($managedConfig.GrantAudience)",
    "signingKeyModulus": "$($managedConfig.SigningKeyModulus)",
    "signingKeyExponent": "$($managedConfig.SigningKeyExponent)",
}
Path("`$CONFIG_PATH").write_text(json.dumps(config, separators=(",", ":")))
PY

/usr/sbin/chown root:wheel "`$CONFIG_PATH"
/bin/chmod 600 "`$CONFIG_PATH"
"@ | Set-Content -Path $helperConfigPath -Encoding utf8
    ...
    [pscustomobject]@{
        ...
        IntunePostInstallScriptPath = $helperConfigPath
